...
function safeUrl(raw: string, image = false) {
  ...
  try {
    ...
    return SAFE_PROTOCOLS.includes(url.protocol) &&
      (!image || ["http:", "https:"].includes(url.protocol))
      ? raw
      : "";
  } catch {
    return "";
...
export function sanitizeTemplateHtml(input: string) {
  let html = input
    .replace(
      /<\s*(script|iframe|object|embed|svg|math|form|input|button|textarea|select|meta|link|style)\b[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/gi,
      "",
    )
    .replace(
      /<\s*(script|iframe|object|embed|svg|math|form|input|button|textarea|select|meta|link|style)\b[^>]*\/?>/gi,
      "",
    )
