...
export async function GET(
  ...
  try {
    ...
    const pageNumber = doc.pageNumbers
      ...
      : "";
    const headerTemplate = `<div style="font-size:9px;width:100%;padding:0 20mm;color:#64748b">${doc.renderedHeader ?? ""}</div>`;
    const footerTemplate = `<div style="font-size:9px;width:100%;padding:0 20mm;color:#64748b;display:flex;justify-content:space-between"><span>${doc.renderedFooter ?? ""}</span><span>${pageNumber}</span></div>`;
